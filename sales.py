...
@sales.route('/add_location', methods=['GET', 'POST'])
def add_location():
    if request.method == 'GET':
        # Render the template for adding a location
        ...
        # Save the location name in the database (replace with your logic)
        # ... (Implement logic to save location_name in your database)

        
        existing_location = locations.query.filter_by(locationName=location_name).first()
        if existing_location:
            
            flash ('this Location is already registerd')
            return redirect(url_for('sales.add_location'))
        else:
            new_location = locations(locationName=location_name)
...
def create_sale():
    ...
    db.session.commit()
    print('all done')
    return jsonify({'message': 'Sale created successfully', 'sale_id': sale.orderid})
    #return jsonify({'message': 'Sale created successfully'})



@sales.route('/api/sales/pdf/<int:sale_id>', methods=['GET'])
def generate_sale_pdf(sale_id):
    sale = Sales.query.get_or_404(sale_id)
    items = sales_Items.query.filter_by(sale_id=sale.orderid).all()
    ...
    p.drawString(100, 750, f"Sales Receipt - Order ID: {sale.orderid}")
    p.drawString(100, 730, f"Customer ID: {sale.customerID}")
    p.drawString(100, 710, f"Salesman ID: {sale.salesmanID}")
    p.drawString(100, 690, f"Total Amount: {sale.total_amount}")
 
    y_position = 650
    for item in items:
        p.drawString(100, y_position, f"Product ID: {item.product_id}, Quantity: {item.quantity}, Profit: {item.profit}")
        y_position -= 20
    ...
    buffer.seek(0)
    return send_file(buffer, as_attachment=True, mimetype='application/pdf', download_name=f'sale_{sale_id}.pdf')


@sales.route('/api/sales/list', methods=['GET'])
def list_sales():
    ...
    sales_list = [{
        ...
        'selldate': sale.selldate.strftime('%Y-%m-%d %H:%M:%S'),
        'total_amount': sale.total_amount
    } for sale in sales]
    return jsonify(sales_list)

@sales.route('/invoice_list')
def invoice_list():
